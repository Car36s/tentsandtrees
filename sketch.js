...
function draw() {
  ...
  if (
    ...
    strokeWeight(3);
    rect(
      highlighted.x * side + 3,
      highlighted.y * side + 3,
      side - 6,
      side - 6
    );
  }
...
function generateTentsAndTrees() {
  ...
  while (openFields.length > size && noMoreSpace < 10) {
    ...
    const fieldToSet = fields.find(
      (field) => field.id === openFields[newTent].id
    );
    ...
    for (let ii = 1; ii > -2; ii--) {
      for (let jj = 1; jj > -2; jj--) {
        const openFieldToFill = fields.find(
          (field) => field.id === getId(fieldToSet.x + ii, fieldToSet.y + jj)
        );
        ...
        if (openFieldToFill) {
          ...
          const index = openFields.findIndex(
            (field) => field.id === openFieldToFill.id
          );
          if (index > -1) {
  ...
  openFields.forEach((field) =>
    fields.find((f) => field.id === f.id).setFieldType("unchecked")
  );
}
...
function getNeighbouringTreeId(openFields, current) {
  ...
  for (let ii = -1; ii < 2; ii += 2) {
    for (let jj = -1; jj < 2; jj += 2) {
      let fieldToTree;
      if (ii === -1) {
        fieldToTree = openFields.find(
          (field) => field.id === getId(current.x, current.y + jj)
        );
      } else {
        fieldToTree = openFields.find(
          (field) => field.id === getId(current.x + jj, current.y)
        );
      }
